...
const BestProducts = () => {
    return (
        <section className='w-full py-10 bg-gray-100 '>
            ...
            <main className="sm:grid sm:grid-cols-2 justify-center place-content-center flex flex-wrap items-center px-6 overflow-x-hidden gap-4 md:gap-6 lg:grid-cols-3 xl:grid-cols-4">
                {Bestperfumes.map((perfume) => (
                    <Link href={`perfumes/${perfume.title.replaceAll(' ',"")}`} key={perfume.title} className='w-[360px] bg-gray-100 border sm:w-72 md:w-[340px] lg:w-72 flex flex-col shadow-lg gap-2 rounded-lg '  >
